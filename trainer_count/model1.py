...
def model_fn(features, labels, mode, params):
    ...
    Y1 = layer_conv2d_batch_norm_relu(X,  filters=8, kernel_size=6, strides=1)
    Y2 = layer_conv2d_batch_norm_relu(Y1, filters=16, kernel_size=5, strides=2)
    Y3 = layer_conv2d_batch_norm_relu(Y2, filters=8, kernel_size=5, strides=1)
    Y4 = layer_conv2d_batch_norm_relu(Y3, filters=16, kernel_size=4, strides=2)
    Y5 = layer_conv2d_batch_norm_relu(Y4, filters=8, kernel_size=4, strides=1)
    Y6 = layer_conv2d_batch_norm_relu(Y5, filters=16, kernel_size=4, strides=2)
    Y7 = layer_conv2d_batch_norm_relu(Y6, filters=8, kernel_size=4, strides=1)
    Y8 = layer_conv2d_batch_norm_relu(Y7, filters=16, kernel_size=4, strides=2)
    Y9 = layer_conv2d_batch_norm_relu(Y8, filters=4, kernel_size=1, strides=1)
    Y10 = tf.reshape(Y9, [-1, 16*16*4])
    Y11 = layer_dense_batch_norm_relu_dropout(Y10, params['dense'], params['dropout'])
...
def model_fn2(features, labels, mode, params):
    ...
    Y15 = layer_conv2d_batch_norm_relu(Y14, filters=N_CLASSES, kernel_size=1, strides=1) # output 8*8*N_CLASSES
    Ylogits = tf.reduce_mean(Y15, [1,2]) # global average pooling: output 1*1*N_CLASSES shape [batch, N_CLASSES]
    predict = tf.nn.softmax(Ylogits)
    classes = tf.cast(tf.argmax(predict, 1), tf.int32)
...
def model_fn_squeeze(features, labels, mode, params):
    ...
    # maxpool
    Y2 = tf.layers.max_pooling2d(Y1, pool_size=3, strides=2)                    # output 64x64x32
    ...
    Y6 = tf.concat([Y6l, Y6t], 3)                                                # output 64x64x64
    # maxpool
    Y9 = tf.layers.max_pooling2d(Y6, pool_size=3, strides=2)                     # output 32x32x64
    Y10 = layer_conv2d_batch_norm_relu(Y9, filters=32, kernel_size=1, strides=1) #squeeze
    Y11l = layer_conv2d_batch_norm_relu(Y10, filters=64, kernel_size=1, strides=1) #expand 1x1
    Y11t = layer_conv2d_batch_norm_relu(Y10, filters=64, kernel_size=3, strides=1) #expand 3x3
    ...
    #maxpool
    Y16 = tf.layers.max_pooling2d(Y13, pool_size=3, strides=2)                     # output 16x16x32
    Y17 = layer_conv2d_batch_norm_relu(Y16, filters=16, kernel_size=1, strides=1) #squeeze
    Y17l = layer_conv2d_batch_norm_relu(Y17, filters=32, kernel_size=1, strides=1) #expand 1x1
    Y17t = layer_conv2d_batch_norm_relu(Y17, filters=32, kernel_size=3, strides=1) #expand 3x3
    Y18 = tf.concat([Y17l, Y17t], 3)                                              # output 16x16x32
    Y19 = layer_conv2d_batch_norm_relu(Y18, filters=N_CLASSES, kernel_size=1, strides=1) # output 16*16*N_CLASSES
    Ylogits = tf.reduce_mean(Y19, [1,2]) # global average pooling: output 1*1*N_CLASSES shape [batch, N_CLASSES]
    predict = tf.nn.softmax(Ylogits)
    classes = tf.cast(tf.argmax(predict, 1), tf.int32)
...
def model_fn_squeeze2(features, labels, mode, params):
    ...
    Y17t = layer_conv2d_batch_norm_relu(Y17, filters=32, kernel_size=3, strides=1) #expand 3x3
    Y18 = tf.concat([Y17l, Y17t], 3)                                              # output 16x16x32
    Y19 = layer_conv2d_batch_norm_relu(Y18, filters=N_CLASSES, kernel_size=1, strides=1) # squeeze to 16*16*N_CLASSES
    Ylogits = tf.reduce_mean(Y19, [1,2]) # global average pooling: output 1*1*N_CLASSES shape [batch, N_CLASSES]
    predict = tf.nn.softmax(Ylogits)
    classes = tf.cast(tf.argmax(predict, 1), tf.int32)
